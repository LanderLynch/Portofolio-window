...
function getPageZoomScale() {
  ...
  const cssZoom = Number.parseFloat(
    getComputedStyle(document.body).getPropertyValue('--page-zoom')
  );
...
function normalizePointer(clientX, clientY) {
  const zoomScale = getPageZoomScale();
...
function storePointerPosition(clientX, clientY) {
...
window.addEventListener('mousemove', (event) => {
  storePointerPosition(event.clientX, event.clientY);
}, { passive: true });

window.addEventListener('touchstart', (event) => {
  const touch = event.touches?.[0];
  if (touch) {
    storePointerPosition(touch.clientX, touch.clientY);
  }
}, { passive: true });
...
class Creature {
  constructor(containerId, spriteConfig) {
    ...
    this.pendingFollowUpAction = null;
    this.currentFrame = 0;
    ...
    this.positionX = Math.random() * (viewportWidth - this.containerWidth);
    this.positionY = viewportHeight - this.containerHeight;

    this.container.style.left = `${this.positionX}px`;
    this.container.style.top = `${this.positionY}px`;

    this.maxPos = viewportWidth - this.containerWidth; // max horizontal position
    ...
    this.resizeHandler = () => {
      ...
      this.maxPos = viewportWidth - this.containerWidth;
  ...
  facePosition(targetX) {
    this.setFacingFromDelta(targetX - this.positionX);
  }

  chooseWeightedAction(actionPool = []) {
    const validPool = actionPool.filter((entry) => entry && entry.name && entry.weight > 0);
    ...
    return validPool[validPool.length - 1].name;
  }

  playTimedLoop(frames, interval, duration, onComplete) {
    let frameIndex = 0;
    ...
    if (frames.length > 1) {
      this.frameTimer = setInterval(() => {
        ...
        this.img.src = frames[frameIndex];
      }, interval);
    }
  ...
  jumpToEdge(targetEdge) {
    ...
    switch (targetEdge) {
        case 'top':
            endY = 0;
            endX = Math.random() * (viewportWidth - this.containerWidth); // random horizontal
            break;
        case 'left':
            endX = 0;
            endY = Math.random() * (viewportHeight - this.containerHeight); // random vertical
            break;
        case 'right':
            endX = viewportWidth - this.containerWidth;
            endY = Math.random() * (viewportHeight - this.containerHeight); // random vertical
            break;
    }
  ...
  enableDragInteraction() {
    ...
    const onPointerMove = (e) => {
        ...
        const { x: clientX, y: clientY, zoomScale } = normalizePointer(pointerX, pointerY);

        this.positionX = clientX - offsetX;
        this.positionY = clientY - offsetY;

        // clamp position to window
        this.positionX = Math.max(0, Math.min(this.positionX, (window.innerWidth / zoomScale) - this.containerWidth));
        this.positionY = Math.max(0, Math.min(this.positionY, (window.innerHeight / zoomScale) - this.containerHeight));
    ...
    const { x: normalizedClientX, y: normalizedClientY, zoomScale } = normalizePointer(clientX, clientY);
    const normalizedRectLeft = rect.left / zoomScale;
    const normalizedRectTop = rect.top / zoomScale;
    let offsetX = normalizedClientX - normalizedRectLeft;  // remember grab offset
    let offsetY = normalizedClientY - normalizedRectTop;
  ...
  fallToBottom(fallSpeed=this.spriteConfig.fallspeed){
    ...
    const { height: viewportHeight } = getViewportBounds();
    const startY=this.positionY, endY=viewportHeight-this.containerHeight, distance=endY-startY;
  ...
  setNextAction() {
    ...
    const { height: viewportHeight } = getViewportBounds();
    if (!this.isJumping && this.positionY >= viewportHeight - this.containerHeight) {
    ...
    if (this.pendingFollowUpAction) {
      const nextAction = this.pendingFollowUpAction;
      this.pendingFollowUpAction = null;
  ...
  startChaseMouse() {
    ...
    if (this.currentEdge !== 'bottom') {
      this.pendingFollowUpAction = 'chaseMouse';
      this.fallToBottom();
      return;
    }
    ...
    const chaseConfig = this.spriteConfig.chaseMouse || this.spriteConfig.walk;
    const targetX = Math.max(0, Math.min(pointer.x - (this.containerWidth / 2), this.maxPos));
    const deltaX = targetX - this.positionX;

    if (Math.abs(deltaX) < 6) {
      this.facePosition(targetX);
      this.pendingFollowUpAction = 'sitAndFaceMouse';
      this.setNextAction();
      return;
    }

    this.currentAction = 'chaseMouse';
    this.resetAnimation();
    this.facePosition(targetX);

    let frame = 0;
    this.img.src = chaseConfig.frames[0];
    this.frameTimer = setInterval(() => {
      frame = (frame + 1) % chaseConfig.frames.length;
      this.img.src = chaseConfig.frames[frame];
    }, chaseConfig.interval);

    const startX = this.positionX;
    const duration = Math.abs(deltaX) / this.spriteConfig.walkspeed;
    const startTime = performance.now();
    ...
    const step = (time) => {
      ...
      this.positionX = startX + (deltaX * progress);
      ...
      this.applyEdgeOffset();
      this.pendingFollowUpAction = Math.random() < 0.05 ? 'trip' : 'sitAndFaceMouse';
      this.setNextAction();
    };
  ...
  completeBottomAction(action) {
    if (action === 'stand' || action === 'sitAndLookUp') {
      this.setNextAction();
      return;
    }

    if (action === 'sit') {
      this.pendingFollowUpAction = this.chooseWeightedAction(this.spriteConfig.SIT_DOWN_NEXT_ACTIONS);
      this.setNextAction();
      return;
    }

    if (action === 'sitAndFaceMouse') {
      this.pendingFollowUpAction = this.chooseWeightedAction(this.spriteConfig.SIT_FACE_NEXT_ACTIONS);
      this.setNextAction();
      return;
    }

    if (action === 'sitAndSpinHead') {
      this.pendingFollowUpAction = 'sitAndFaceMouse';
      this.setNextAction();
      return;
    }
    ...
    if (action === 'lieDown') {
      this.pendingFollowUpAction = this.chooseWeightedAction(this.spriteConfig.LIE_DOWN_NEXT_ACTIONS);
      this.setNextAction();
      return;
    }

    if (action === 'crawlAlongFloor') {
      this.pendingFollowUpAction = 'lieDown';
      this.setNextAction();
      return;
    }
  ...
  startAction(action) {  
    ...
    if (action === 'chaseMouse') {
      this.startChaseMouse();
      return;
    }
    ...
    if (action === 'sitAndFaceMouse') {
      const pointer = getLastPointerPosition();
      if (pointer) {
        this.facePosition(pointer.x);
      }
    }

    if (action === 'stand' || action === 'sit' || action === 'sitAndFaceMouse' || action === 'sitAndLookUp' || action === 'hangstillSide' || action === 'hangstillTop' || action === 'lieDown') {
      const duration = config.randomizeDuration
        ? Math.random() * (config.max - config.min) + config.min
        : interval * loops;
      this.playTimedLoop(frames, interval, duration, () => this.completeBottomAction(action));
      return;
    }

    if (action === 'sitWhileDanglingLegs') {
      this.playAnimation(frames, interval, loops, () => this.completeBottomAction(action));
      return;
    }

    if (action === 'crawlAlongFloor') {
      this.direction = Math.random() < 0.5 ? -1 : 1;
      this.setFacingFromDelta(this.direction);
      this.playAnimation(frames, interval, loops, () => this.completeBottomAction(action));
      return;
    }
  ...
  animate(time) {
    ...
    const movingActions = ['walk', 'run', 'dash', 'forced-walk', 'climbTop', 'crawlAlongFloor']; // add actions with horizontal movement here
    if (movingActions.includes(this.currentAction)) {
        const speedMultiplier =
          this.currentAction === 'crawlAlongFloor' ? 0.35 :
          this.currentAction === 'run' ? 2 :
          this.currentAction === 'dash' ? 4 : 1;
        const dx = this.direction * this.spriteConfig.walkspeed * speedMultiplier * delta;
        this.positionX += dx;
    ...
    if (this.currentAction === 'climbSide') {
      ...
      // flips sprites and movement direction upon reaching a wall
      const { height: viewportHeight } = getViewportBounds();
      const maxY = viewportHeight - this.containerHeight;
      if (this.positionY <= 0) {
        this.positionY = 0;
        this.direction = 1;
